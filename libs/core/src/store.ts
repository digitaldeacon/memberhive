...
export const isAwaitingFormSubmit: any = createSelector(getSettingsState, settings.awaitingFormSubmit);
/**
        ...
        if (fieldSearch.length === 1) {
          ...
          if (filterSet.key === 'age') {
            ...
            if (exprRange.length > 1) {
              found =
                prs[filterSet.key] >= parseInt(exprRange[0], 10) && prs[filterSet.key] <= parseInt(exprRange[1], 10);
            } else {
